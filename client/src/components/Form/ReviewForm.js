...
import {
  Box, Typography, List, ListItem, ListItemText, Grid,
} from '@mui/material';
...
export default function Review({ cart, user, payment }) {
  ...
  const getAddress = () => [`${user.address.street}, ${user.address.postalCode} ${user.address.city}, ${user.address.country}`];
        ...
        <Grid item container direction="column" xs={12} sm={6}>
          ...
          <Grid container>
            {payments.map((p) => (
              <Fragment key={p.name}>
                <Grid item xs={6}>
                  <Typography gutterBottom>{p.name}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography gutterBottom>{p.detail}</Typography>
                </Grid>
              </Fragment>
            ))}
          </Grid>
